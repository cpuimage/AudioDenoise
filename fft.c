...
static void BackwardFFT(fft_plan p) {
    if (p.c_out == NULL) {  // c2r
        p.input[0] = p.c_in[0].real;
        p.input[1] = p.c_in[p.n / 2].real;
        for (int i = 1; i < p.n / 2; ++i) {
            p.input[i * 2] = p.c_in[i].real;
            p.input[i * 2 + 1] = -p.c_in[i].imag;
        }
        rdft(p.n, -1, p.input, p.ip, p.w);
        for (int i = 0; i < p.n; ++i)
            p.out[i] = p.input[i] * 2.0f;
    } else {  // c2c
        for (int i = 0; i < p.n; ++i) {
            p.input[i * 2] = p.c_in[i].real;
            p.input[i * 2 + 1] = p.c_in[i].imag;
        ...
        for (int i = 0; i < p.n; ++i) {
            p.c_out[i].real = p.input[i * 2];
            p.c_out[i].imag = -p.input[i * 2 + 1];
        }
    }
}
...
static void ForwardFFT(fft_plan p) {
    if (p.c_in == NULL) {  // r2c
        ...
        rdft(p.n, 1, p.input, p.ip, p.w);
        p.c_out[0].real = p.input[0];
        p.c_out[0].imag = 0.0;
        for (int i = 1; i < p.n / 2; ++i) {
            p.c_out[i].real = p.input[i * 2];
            p.c_out[i].imag = -p.input[i * 2 + 1];
        }
        p.c_out[p.n / 2].real = p.input[1];
        p.c_out[p.n / 2].imag = 0.0;
    } else {  // c2c
        for (int i = 0; i < p.n; ++i) {
            p.input[i * 2] = p.c_in[i].real;
            p.input[i * 2 + 1] = p.c_in[i].imag;
...
fft_plan fft_plan_dft_1d(size_t n, fft_complex *in, fft_complex *out, int sign,
    ...
    output.sign = sign;
    output.flags = flags;
    output.input = (float *) calloc((n * 2), sizeof(float));
    output.ip = (int *) calloc(n, sizeof(int));
    output.w = (float *) calloc((n * 5 / 4), sizeof(float));
    if (output.input == NULL || output.ip == NULL || output.w == NULL) {
        if (output.input) free(output.input);
        if (output.ip) free(output.ip);
        if (output.w) free(output.w);
        output.w = NULL;
        output.input = NULL;
        output.ip = NULL;
    } else {
...
fft_plan fft_plan_dft_c2r_1d(size_t n, fft_complex *in, float *out,
    ...
    output.flags = flags;
    output.input = (float *) calloc(n, sizeof(float));
    output.ip = (int *) calloc(n, sizeof(int));
    output.w = (float *) calloc((n * 5 / 4), sizeof(float));
    if (output.input == NULL || output.ip == NULL || output.w == NULL) {
        if (output.input) free(output.input);
